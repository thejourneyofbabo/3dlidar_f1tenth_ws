    ...
    pub fn new(executor: &Executor) -> Result<Self, RclrsError> {
        ...
        let scan_subscription =
            node.create_subscription::<LaserScan, _>("/scan", move |msg: LaserScan| {
                if let Err(e) =
                    Self::process_scan(msg, &publisher_clone, &prev_best_clone, &is_active_scan)
                {
    ...
    fn mission_callback(msg: StdString, is_active: &Arc<Mutex<bool>>) {
        ...
        *active_lock = msg.data == "MISSION_B";
        ...
        if *active_lock != was_active {
            if *active_lock {
                println!("Gap Following node activated - Mission B");
            } else {
                println!("Gap Following node deactivated");
            }
        }
    }
...
fn main() -> Result<(), RclrsError> {
    println!("F1Tenth Gap Follow Node - Enhanced Sigmoid Version (Mission Based)");
