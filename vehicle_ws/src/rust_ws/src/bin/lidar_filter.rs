...
struct LidarFilteredPublisher {
    ...
    // Publisher is used in the callback, but we don't need to store it here
    // since it's already captured in the closure
}
...
impl LidarFilteredPublisher {
    ...
    fn new(node: &Arc<Node>) -> Result<Self> {
        ...
        let filtered_publisher =
            Arc::new(node.create_publisher::<PointCloud2>(
                "/livox/lidar_filted",
                rclrs::QOS_PROFILE_DEFAULT,
            )?);

        // Create original LiDAR subscriber with callback
        // create_subscription already returns Arc<Subscription<T>>, no need to wrap again
        let scan_subscription = node.create_subscription::<PointCloud2, _>(
            "/livox/lidar",
            rclrs::QOS_PROFILE_DEFAULT,
            move |msg: PointCloud2| {
                if let Err(e) = Self::process_and_publish_filtered(msg, &filtered_publisher) {
