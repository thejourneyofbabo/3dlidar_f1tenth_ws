...
use std_msgs::msg::String as StdString;
...
struct ReactiveFollowGap {
    ...
    mission_subscription: Subscription<StdString>,
...
impl ReactiveFollowGap {
    fn new(executor: &Executor) -> Result<Self, RclrsError> {
        let node = executor.create_node("reactive_node")?;
        ...
        let is_active_scan = is_active.clone();
        let is_active_mission = is_active.clone();

        // Mission subscription
        let mission_subscription =
            node.create_subscription::<StdString, _>("current_mission", move |msg: StdString| {
                Self::mission_callback(msg, &is_active_mission);
            })?;
        ...
        let scan_subscription =
            node.create_subscription::<LaserScan, _>("/scan", move |msg: LaserScan| {
                if let Err(e) =
                    Self::lidar_callback(msg, &publisher_clone, &prev_best_clone, &is_active_scan)
                {
    ...
    fn mission_callback(msg: StdString, is_active: &Arc<Mutex<bool>>) {
        let mut active_lock = is_active.lock().unwrap();
        let was_active = *active_lock;
        *active_lock = msg.data == "MISSION_B";

        if *active_lock != was_active {
            if *active_lock {
                println!("LiDAR node activated - Mission B");
            } else {
                println!("LiDAR node deactivated");
            }
        }
    }
    ...
    fn lidar_callback(
        ...
        // Always process LiDAR data (for debugging/monitoring purposes)
        ...
        // Only publish drive command if in Mission B
        let active_lock = is_active.lock().unwrap();
        if *active_lock {
...
fn main() -> Result<(), RclrsError> {
    println!("F1Tenth Gap Follow Node with Rust - Mission Based");
