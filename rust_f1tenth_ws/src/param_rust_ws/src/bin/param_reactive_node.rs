// src/bin/param_reactive_node.rs
...
// 라이브러리에서 파라미터 구조체 가져오기
use param_rust_ws::VehicleParams;
...
impl ReactiveFollowGap {
    fn new(executor: &Executor) -> Result<Self, RclrsError> {
        ...
        // 파라미터 로드 - YAML 파일 지원
        let params = VehicleParams::from_ros_params(&node)?;
...
fn main() -> Result<(), RclrsError> {
    println!("F1Tenth Gap Follow Node with Rust (Simple Parameter Library)");
