...
impl ReactiveFollowGap {
    ...
    fn find_max_gap(msg: &LaserScan, ranges: &[f32]) -> (usize, usize) {
        ...
        let roi_angle_steps = (roi_angle_rad / msg.angle_increment) as usize; // ROI angle steps
                                                                              // for left & right
        let mid_lidar_idx =
            ((msg.angle_max - msg.angle_min) / (msg.angle_increment * 2.0)) as usize;
        ...
        for i in roi_idx_start..=roi_idx_end {
            ...
            // Gap end
            if (!is_free && was_free) || (is_free && i == roi_idx_end) {
                let gap_end = if is_free { i } else { i - 1 };
    ...
    fn vehicle_control(msg: &LaserScan, best_point: usize) -> (f32, f32) {
        ...
        let vehicle_center_idx =
            ((msg.angle_max - msg.angle_min) / (msg.angle_increment * 2.0)) as usize;
        let steer_ang: f32 =
            (best_point as f32 - vehicle_center_idx as f32) * msg.angle_increment * 180.0 / PI;

        let drive_speed = match steer_ang {
            n if n < 5.0 => 2.0,
            n if n < 10.0 => 1.5,
            n if n < 15.0 => 1.2,
            _ => 0.8,
